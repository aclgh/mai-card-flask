...
@app.route("/")
def index():
    return jsonify({"status": "200 OK", "timestamp": int(time.time()), "info": "Mai Card Flask", "availableApi": ["gen"], "date": datetime.now(pytz.timezone('Asia/Shanghai')).strftime('%a, %d %b %Y %H:%M:%S GMT+8')}), 200
